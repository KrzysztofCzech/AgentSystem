...
public class FixApp extends JFrame {
    ...
    JLabel label2;
    JLabel label4;
    JList<String> listInvitedPersons;
    DefaultListModel<String> invitedModel;
    JList<String> listKnownPersons;
    DefaultListModel<String> knownModel;
    JLabel label1;
    JLabel label3;
    ...
    public FixApp(MeetingSchedulerAgent a, Calendar selectedDate) {
        ...
        Enumeration<Person> e = myAgent.getKnownPersons();
        knownModel.clear();
        int k = 0;
        while (e.hasMoreElements()) {
            knownModel.addElement(e.nextElement().getName());
        }
    ...
    void buttonExit_MouseClicked(MouseEvent event) {
        // Invalidate the Frame
        dispose();
    }

    void buttonOk_MouseClicked(MouseEvent event) {
        Calendar c;
        Date d;
        ...
        for (int i = 0; i < invitedModel.getSize(); i++)
            a.addInvitedPersons(myAgent.getPerson(invitedModel.get(i)));
        try {
            a.isValid();
            GuiEvent ev = new GuiEvent(this, MeetingSchedulerAgent.FIXAPPOINTMENT);
            ev.addParameter(a);
    ...
    void buttonAddPerson_MouseClicked(MouseEvent event) {
    ...
    void buttonRemovePerson_MouseClicked(MouseEvent event) {
